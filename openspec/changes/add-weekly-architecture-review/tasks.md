...
- [x] 6.3 With `GITHUB_REPO`/`ARCH_REVIEW_REPO` pointed at a sandbox repository, trigger one run (`wrangler dev --test-scheduled` against `/__scheduled`, or call `dispatchArchitectureReview` directly) and confirm: the run dispatches, the agent reads real files, and exactly one labelled issue is filed with evidenced findings. — *adapted: the fine-grained PAT only grants issue-write on the real tracker, so the tracker stayed `Stock-GPT/marketsavvy` (test issue #497, filed with 7 `path:line` citations, closed after verification) while `ARCH_REVIEW_REPO=Nfinger/bug-triage-agent` was reviewed; fired via wrangler's `/cdn-cgi/handler/scheduled` endpoint*
- [x] 6.4 Confirm idempotency: fire again for the same `runDate` and verify no second run and no second issue (Cloudflare delivers scheduled events at-least-once). — *found and fixed two bugs: (1) a same-day refire crashed `scheduled()` with an uncaught idempotency-key error (now caught and logged as a no-op in `dispatchArchitectureReview`); (2) a refire admitted after a restart re-ran the agent and filed a duplicate (#498, closed) — `file_architecture_report_issue` now derives the title from run context and returns the existing issue for the run date instead of creating another; re-verified: refire settles with "already filed for this run" and the issue count is unchanged*
- [x] 6.5 Confirm failure handling: run once with an invalid `GITHUB_TOKEN` and verify the failed tool result appears in the conversation and the agent does not claim success. — *verified: all GitHub reads recorded 401 "Bad credentials" tool failures in the conversation, the filing tool was never called, no issue was created, and no success was reported*
...
- [x] 6.8 Point `GITHUB_REPO`/`ARCH_REVIEW_REPO` back at their real values after the sandbox run. — *local `.dev.vars` restored (`GITHUB_REPO=Stock-GPT/marketsavvy`, `ARCH_REVIEW_REPO=Nfinger/bug-triage-agent`); also set the missing `ARCH_REVIEW_REPO` secret on the deployed Worker, which otherwise defaulted to reviewing the tracker repo while the focus briefs describe this app*
